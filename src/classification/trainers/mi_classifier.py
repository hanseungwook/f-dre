...
class MIClassifier(BaseTrainer):
    ...
    def plot_logratios(self, num_samples=1000, epoch=0):
        ...
        scat1 = ax2.scatter(samples.cpu(),true_kl,label='True Log p/q, KL = '+str(np.around(true_kl.cpu().mean().item(),2)),alpha=0.9,s=10.,c='b')
        scat2 = ax2.scatter(samples.cpu(),est_kl,label='CoB Log p/q, KL = '+str(np.around(est_kl.cpu().mean().item(),2)),alpha=0.9,s=10.,c='r')
